...
> 建议先读开头两节，先把协议对象和资金流串起来，再进入公式和协议细节。
...
0. [开始前先看](#开始前先看)
1. [先建立 DeFi 的资产流转视角](#1-先建立-defi-的资产流转视角)

### DEX 与交易
2. [DeFi 概述](#1-defi-概述)
3. [AMM 自动做市商](#2-amm-自动做市商)
4. [无常损失](#3-无常损失)
...
DeFi 入门时常见的问题是：只盯着协议名字和 APY，却没有先看清“资产怎么流、谁承担风险、规则由谁执行”。
...
DeFi 的本质不是“去中心化 UI”，而是“用公开可验证的合约规则重写交易、借贷、稳定币和清算流程”。
...
### 1.0.1 为什么 DeFi 一定要先看资金流

因为 DeFi 很多概念如果不放在资金流里看，会变成空词：

- `AMM`：本质上是在池子里重新定价和换资产
- `借贷`：本质上是在抵押和负债之间做动态平衡
- `稳定币`：本质上是在维护某种价格锚定
- `清算`：本质上是在风险过界时强制平仓
- `MEV`：本质上是在交易排序上攫取价值

所以你后面看到的所有公式，都应该回到一个问题：它到底在约束哪种资金流和风险暴露。
