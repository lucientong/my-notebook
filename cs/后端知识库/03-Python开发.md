...
- **引用计数**：每个对象维护引用计数，归零时立即回收
- **垃圾回收**：标记-清除、分代回收（解决循环引用）
- **内存池**：小对象复用（PyMalloc）

---

**2. 什么是装饰器？如何实现带参数的装饰器？**

装饰器是一个接受函数作为参数并返回新函数的函数。带参数的装饰器需要多一层嵌套。
...
GIL 是全局解释器锁，限制同一时间只有一个线程执行 Python 字节码。

绕过方式：
- CPU 密集型：使用多进程
- IO 密集型：使用多线程或 asyncio
