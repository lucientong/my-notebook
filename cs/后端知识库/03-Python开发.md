...
**1. Python 里的可变对象和不可变对象有哪些？为什么这件事重要？**

常见不可变对象有 `int`、`float`、`str`、`tuple`，常见可变对象有 `list`、`dict`、`set`。这件事重要，是因为它直接影响函数默认参数、对象共享引用、拷贝语义以及多处代码是否会互相污染状态。

**2. `list` 和 `tuple` 的区别是什么？什么时候优先用 `tuple`？**

`list` 可变，适合动态增删改；`tuple` 不可变，更轻量，也更适合作为字典 key 或表达“这一组值不该被改”。如果数据天然是固定结构且只读，优先用 `tuple` 会更准确地表达语义。

**3. `*args` 和 `**kwargs` 分别是什么？**

`*args` 用来接收额外的位置参数，表现为一个元组；`**kwargs` 用来接收额外的关键字参数，表现为一个字典。很多框架 API 设计喜欢用它们，但面试里更看重你是否知道“灵活”背后的可读性代价。

**4. `@staticmethod`、`@classmethod`、实例方法三者有什么区别？**

实例方法依赖对象实例，第一个参数通常是 `self`；类方法依赖类本身，第一个参数是 `cls`，常用于工厂方法；静态方法只是挂在类命名空间下的普通函数，不依赖实例也不依赖类。选错会让接口语义变得别扭。

**5. 列表推导式和生成器表达式的区别是什么？**

列表推导式会一次性生成结果，适合数据量可控、后续需要多次访问的场景；生成器表达式惰性求值，更省内存，适合流式处理和大数据量遍历。很多性能问题本质上就是把本该用生成器的地方写成了列表推导式。

### 中级高频进阶

**6. 为什么 Python 里不建议把 `[]` 或 `{}` 作为函数默认参数？**

因为默认参数在函数定义时就会求值，后续多次调用共享的是同一个对象。这样一个调用里对列表的修改，会污染下一次调用的默认值。这是 Python 面试里命中率极高的经典题。

**7. Python 的传参到底更像值传递还是引用传递？**

更准确地说，是对象引用被传入函数。函数内部如果只是给形参重新赋值，不影响外部；但如果修改了形参指向的可变对象内容，外部会看到变化。所以面试里直接回答“值传递”或“引用传递”都不够严谨。

**8. 为什么 `async def` 里不能直接跑 CPU 密集型任务？**

因为 `asyncio` 的核心是单线程事件循环，CPU 密集型计算会长时间占住线程，不主动让出控制权，导致整个事件循环卡住。正确做法通常是把 CPU 密集逻辑扔到进程池或 C 扩展里。
...
#### Q21：什么是描述符（Descriptor）？`property`、实例方法和 ORM 字段为什么都和它有关？

**答**：描述符是实现了 `__get__`、`__set__`、`__delete__` 中一个或多个方法的对象，Python 在属性查找时会优先检查类属性里是否存在描述符。`property` 通过描述符把方法包装成受控属性访问；函数定义在类中后，也会通过描述符协议在访问时自动绑定成实例方法；很多 ORM 字段对象也是描述符，用来在读取/赋值时注入校验、延迟加载和脏数据跟踪。面试里真正要答出来的是：描述符不是边角特性，它就是 Python 对象模型里“属性访问可编程”的底层机制。
...
**D1：设计一个Python实现的高性能爬虫框架，需要考虑哪些方面？**

**参考思路**：
- 并发模型：asyncio+aiohttp（IO密集型最佳）、多进程+协程混合（充分利用多核）
- 任务调度：URL去重（布隆过滤器）、优先级队列、分布式任务分发（Celery/Redis队列）
- 反爬对抗：IP池轮转、User-Agent随机、请求频率控制、验证码识别接入
- 数据管道：Pipeline模式（抓取→清洗→存储）、增量更新机制
- 容错：请求超时重试、异常页面跳过、断点续爬（持久化爬取状态）
- 监控：爬取速度/成功率/去重率Dashboard

**D2：Python服务线上内存持续增长不释放，如何系统排查？**

**参考思路**：
- 确认现象：RSS只增不减是Python内存池的正常行为（PyMalloc不归还OS），真正需要关注的是增长速率
- 工具链：tracemalloc（标准库）→ objgraph（对象引用图）→ guppy3（堆分析）
- 常见泄漏源：全局字典/列表无限增长、闭包持有大对象引用、__del__引起循环引用无法GC、C扩展泄漏
- 解决思路：多进程模型（Gunicorn prefork定期重启Worker）、WeakRef弱引用、限制缓存大小（LRU）
