...
三个缩写都来自 Google SRE 体系，按 **测量 → 内部目标 → 对外合同** 层层递进：

| 缩写 | 英文全称 | 中文 | 一句话 |
|------|----------|------|--------|
| **SLI** | Service Level **Indicator** | 服务水平**指标** | 用什么**量**服务质量（可观测、可计算） |
| **SLO** | Service Level **Objective** | 服务水平**目标** | 团队内部承诺 SLI **要达到多少** |
| **SLA** | Service Level **Agreement** | 服务水平**协议** | 写在合同里对客户的承诺，**违约有后果**（赔偿、免责等） |

```text
SLI（测什么）  →  SLO（内部要达标）  →  SLA（对外签合同）
  指标本身          目标阈值               合同条款 + 违约条款
```
...
| 层级 | 定义什么 | 支付 API 示例 |
|------|----------|---------------|
| SLI | **指标定义 + 怎么算** | 「成功支付请求数 / 总支付请求数」，滚动 30 天 |
| SLO | **内部目标线** | 上述成功率 ≥ 99.95%（团队自己定的及格线） |
| SLA | **对客承诺 + 违约处理** | 月可用性 < 99.9% 时，服务费减免 10%（合同条款） |

注意区分 **SLI（指标）** 和 **SLI 的当前测量值**：

| | 是什么 | 示例 |
|--|--------|------|
| SLI 定义 | 指标名称 + 计算公式 + 统计窗口 | `payment_success_rate = success / total`，窗口 30d |
| SLI 测量值 | 按定义算出来的**当前结果** | 过去 30 天成功率 **99.97%** |
| SLO | 测量值应满足的**目标** | 成功率 **≥ 99.95%** |
| SLA | 对客户承诺的**底线**（通常低于 SLO） | 月可用性 **≥ 99.9%**，否则赔偿 |

常见 SLI 类型（都应对准**用户可感知**的质量，而不是机器指标）：

| 类型 | 英文 | 典型 SLI | 不算好的 SLI |
|------|------|----------|--------------|
| 可用性 | Availability | 成功请求占比、健康检查通过率 | CPU < 80% |
| 延迟 | Latency | P99 响应时间、端到端耗时 | 平均 QPS |
| 吞吐 | Throughput | 每秒成功处理订单数 | Pod 副本数 ≥ 3 |
| 正确性 | Correctness | 支付回调按时到达率、数据一致率 | 监控面板全绿 |

| 概念 | 面向谁 | 含义 | 示例 |
|------|--------|------|------|
| **SLI** | 工程 / 可观测 | 可精确测量的服务质量**指标** | 指标：`api_request_success_rate`；当前值：99.97% |
| **SLO** | 内部团队 | 团队承诺 SLI **要达到的目标** | API 成功率 ≥ 99.95%，P99 < 200ms |
| **SLA** | 外部客户 | 合同中的服务承诺，**违约有赔偿或免责** | 月可用性 < 99.9% → 赔偿合同金额 10% |
...
**D1：设计一个千台规模集群的告警系统，如何避免告警风暴和告警疲劳？**

**参考思路**：
- 告警分级：P0(资损/大面积不可用)→P1(核心功能降级)→P2(非核心异常)→P3(性能退化)
- 抑制聚合：相同根因的告警聚合为一条、下游故障时抑制上游告警（依赖关系图）
- 降噪策略：告警阈值动态调整（基线+标准差）、非工作时间只发P0/P1、重复告警间隔递增
- 升级机制：P0 5分钟未ACK自动升级到Leader、30分钟未解决升级到CTO
- 度量：MTTA(平均确认时间)<5min、告警有效率>80%（有效=触发后有人工操作）

**D2：核心服务的SLO从99.95%下降到99.8%，Error Budget即将耗尽，你会采取什么措施？**

**参考思路**：
- 确认消耗原因：分析SLI违约时段、定位根因（是单次大故障还是持续小抖动）
- 短期措施：冻结非紧急变更（Change Freeze）、加强灰度发布比例和观察时间、增加冗余资源
- 技术治理：修复导致SLI下降的Top 3问题、加强混沌工程验证、补全监控盲区
- 流程治理：Post-Mortem复盘不到位的action重新排期、值班SOP补全、自动化回滚覆盖率提升
