...
### Part 2：虚拟机网络
7. [虚拟机网络模式](#1-虚拟机网络模式)
8. [Linux网络虚拟化组件](#2-linux网络虚拟化组件)
9. [虚拟机网络配置实战](#3-虚拟机网络配置实战)
10. [虚拟机迁移技术](#4-虚拟机迁移技术)
11. [VNC远程管理](#5-vnc远程管理)

### Part 3：物理网络与虚拟化网络
12. [物理网络基础](#1-物理网络基础)
13. [VLAN虚拟局域网](#2-vlan虚拟局域网)
14. [网络隧道技术](#3-网络隧道技术)
15. [VXLAN大二层网络](#4-vxlan大二层网络)
16. [VPC虚拟私有云](#5-vpc虚拟私有云)

### Part 4：服务器底层技术
17. [服务器管理技术](#1-服务器管理技术)
18. [高性能网络DPDK](#2-高性能网络dpdk)
19. [高性能存储SPDK](#3-高性能存储spdk)
20. [RDMA远程直接内存访问](#4-rdma远程直接内存访问)
21. [LVM逻辑卷管理](#5-lvm逻辑卷管理)

### Part 5：操作系统运维深入
22. [Linux内核参数调优](#1-linux内核参数调优)
23. [系统排障工具](#2-系统排障工具)
24. [性能分析与监控](#3-性能分析与监控)
25. [文件系统排障](#4-文件系统排障)
26. [网络排障工具](#5-网络排障工具)
27. [内核分析工具](#6-内核分析工具)
...
**D1：设计一个支持百万级虚拟机的云计算调度系统，核心调度算法怎么做？**

**参考思路**：
- 调度维度：CPU/内存/磁盘/网络资源匹配、亲和性/反亲和性约束、AZ容灾分布
- 算法：两级调度（先筛选符合条件的宿主机，再打分排序选最优）、装箱算法（Best-Fit Decreasing）
- 碎片治理：定期热迁移合并碎片、预留Buffer防止过度超分、资源池分层（独占/共享/抢占）
- 性能要求：调度延迟<1s（百万规模需要索引优化，不能遍历所有宿主机）

**D2：物理服务器硬件故障（内存CE错误持续增长）但上面还有业务VM在运行，如何安全处理？**

**参考思路**：
- 评估风险：CE→UE概率评估、memtest86检测故障DIMM位置、判断是否影响VM内存区域
- 在线迁移：优先级排序（核心业务先迁）→ 热迁移到健康宿主机 → 验证迁移后服务正常
- 不可迁移场景：GPU直通VM无法热迁移→协调业务方冷迁移窗口→停机替换硬件
- 自动化：硬件巡检自动发现CE→触发工单→自动调度热迁移→标记宿主机下线维修
