...
**D1：设计一个生产级的RAG系统（检索增强生成），从文档摄入到回答生成全链路怎么做？**

**参考思路**：
- 文档处理：PDF/Docx解析→分块策略（按语义段落/固定Token+Overlap）→ Embedding向量化（BGE/OpenAI）
- 检索层：向量检索（Milvus/Qdrant）+ 关键词检索（BM25）→ RRF融合→ Reranker精排（bge-reranker）
- 生成层：检索结果拼入Prompt → LLM生成 → 引用标注（对应到源文档段落）
- 质量保证：答案归因（每句话标注来源）、拒答机制（检索不到相关内容时不瞎编）、幻觉检测
- 关键指标：Answer Relevance>0.8、Faithfulness>0.9、检索P99<200ms

**D2：Agent在执行复杂任务时经常"跑偏"（脱离用户意图），如何设计约束机制？**

**参考思路**：
- 规划约束：System Prompt中明确边界和禁止操作、限制最大步骤数和最大工具调用次数
- 过程监控：每步执行后检查是否偏离目标（Self-Reflection/Critic Agent）
- 工具安全：工具调用白名单、危险操作需确认、参数范围校验
- 回退机制：连续失败3次自动回退到上一个正确状态、超时强制终止并返回部分结果
- 人机协作：关键决策点暂停等待用户确认、提供中间结果供用户纠偏
