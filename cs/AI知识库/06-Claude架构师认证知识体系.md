...
### Q9: Claude Code 场景里，什么时候应该进入 Planning Mode？

**答案**：
适合进入 Planning Mode 的情况：
1. 变更跨多个模块，影响面大且依赖复杂。
2. 需要明确回滚策略或迁移步骤。
3. 多人协作下需要先对齐执行顺序和边界。

小修小补直接执行更高效；大改动先计划能显著降低返工和冲突。

---

### Q10: 为什么 CLAUDE.md 要做分层（全局/项目/子目录）而不是一份大文件？

**答案**：
分层的价值是把“通用规则”和“局部约束”解耦：
1. 全局层放统一工程规范。
2. 项目层放仓库级流程和测试约束。
3. 子目录层放模块特定规则。

这样可以减少冲突、提高可维护性，也更符合大仓库长期演进。

---

### Q11: 多 Agent 协作里，如何避免上下文污染和错误传播？

**答案**：
1. 子代理上下文最小化，只给完成任务所需信息。
2. 返回结构化结果（状态、证据、建议），避免自由文本扩散歧义。
3. Coordinator 统一做结果合并与冲突裁决。

核心原则：子代理负责产出，Coordinator 负责决策。

---

### Q12: Tool 描述已经很清楚了，为什么仍会选错工具？

**答案**：
常见原因不在描述文本本身，而在工具集合治理：
1. 功能边界重叠，模型难以区分。
2. 参数 schema 相似，导致路由混淆。
3. 缺少真实模糊输入测试，线上才暴露问题。

治理要点是“工具去重 + 边界互斥 + 回归测试”，而不是只优化一版描述。

---

### Q13: 认证备考中，如何把“会背概念”提升到“能过场景题”？

**答案**：
建议用场景模板练习：
1. 先给出架构选择和理由（为什么不用其他方案）。
2. 再给出失败路径和降级策略。
3. 最后给出可观测与回滚方案。

场景题看的是“决策链条完整性”，不是术语数量。

---

### Q14: 如果 Batch API 成本更低，为什么很多场景仍坚持同步调用？

**答案**：
因为同步链路通常承担交互体验和工具多轮编排：
1. 用户等待实时反馈，批处理延迟不可接受。
2. 多轮工具调用需要在线状态推进，Batch 不适配。
3. 故障恢复需要细粒度重试，而不是整批重跑。

成本不是唯一目标，体验和可控性常常更关键。

---

### Q15: 你会如何设计一个“可审计”的 Agent 工作流？

**答案**：
可审计设计至少包含：
1. 每步决策日志（输入、工具、参数、输出、耗时）。
2. 统一 trace_id 串联多代理与多工具调用。
3. 敏感操作的审批与结果留痕。
4. 失败路径可重放，便于复盘和追责。

没有审计能力的自动化，很难在企业场景长期落地。

---

### Q16: 作为架构师，如何判断某个 Agent 方案“现在不该做”？

**答案**：
三个否决信号：
1. 业务目标和评估指标不清，无法定义成功标准。
2. 工具链不稳定，核心依赖无可观测和回滚能力。
3. 团队还没有值班与治理能力，无法承接线上风险。

架构决策不只是“能不能做”，更是“现在做会不会失控”。

---

### 开放式设计题

**D1：设计一个企业级AI助手平台（支持多部门、多场景），从Prompt到部署全链路怎么做？**

**参考思路**：
- 多场景适配：按部门配置不同System Prompt+Tool集合（客服/法务/研发各有专属配置）
- Prompt管理：版本化存储、AB测试框架、效果评估Pipeline
- 知识库集成：RAG架构，各部门独立知识库+全局知识库、权限隔离
- 安全：输入输出Guardrails、PII过滤、审计日志、敏感话题拒答
- 成本控制：模型路由（简单问题用小模型、复杂问题用大模型）、缓存相似问题、Token预算

**D2：企业AI助手出现严重幻觉（给客户提供了错误的退款政策），如何做事后处理和系统性防范？**

**参考思路**：
- 事后处理：紧急下线该场景→修正回复并通知受影响客户→复盘根因
- 系统性防范：RAG强化（回复必须有文档出处引用）、拒答机制（检索不到相关文档时明确说不知道）
- 质量保障：Golden Dataset定期回归、人工抽检制度、用户反馈闭环
- 分级策略：高风险场景（涉及金额/法律/健康）强制人工审核后再发送
