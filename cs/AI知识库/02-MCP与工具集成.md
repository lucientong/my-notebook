...
这篇聚焦“模型如何安全、稳定地连接外部能力”，核心是协议与集成工程，而不是模型原理本身。

- 与 `01-LLM与Agent基础` 的边界：`01` 讲 Agent / Tool Calling 原理，这篇讲 MCP 协议落地细节。
- 与 `03-AI工程化实践` 的边界：`03` 讲系统级治理（成本、SLO、可观测），这篇讲 Tool/Resource 连接层治理。

### 阅读目标

读完后你应该能回答：

- 什么时候该用 Tool，什么时候该用 Resource
- MCP Server 如何做权限隔离、性能优化和故障排查
- 协议升级时如何保持向后兼容，避免客户端大面积不可用

### 建议阅读方式

- 初次接触 MCP：先读 `1-5` 节，建立协议和对象模型。
- 准备上线生产：重点看 `8-10` 节（安全、性能、排障）。
- 面试准备：最后用 `12. 面试题自查` 做分场景回放。
...
### Q16: MCP 工具超时后应该重试吗？如何设计重试策略？

**答案**：
不是所有超时都该重试。建议按错误类型分层：
1. **可重试**：临时网络抖动、上游 5xx、瞬时限流。
2. **不可重试**：参数错误、权限拒绝、业务校验失败。

重试策略：有限次数 + 指数退避 + 抖动，并记录 `retry_count` 到审计日志。避免“无限重试把下游打崩”。

---

### Q17: MCP Server 做多租户时，最容易忽略的隔离点有哪些？

**答案**：
常见漏点有三类：
1. **数据隔离**：缓存 key 未带租户前缀，导致串读。
2. **权限隔离**：工具白名单按全局配置，未做租户差异化策略。
3. **资源隔离**：高流量租户占满连接池，拖慢其他租户。

多租户必须把“鉴权、限流、缓存、审计”都做租户维度隔离。

---

### Q18: 如何做 MCP 协议升级，避免老客户端直接不可用？

**答案**：
升级要遵循“向后兼容优先”：
1. 新增字段而非修改语义，旧字段保留过渡期。
2. 服务端支持多版本协商，按客户端能力降级响应。
3. 升级前做契约测试和回放测试，验证关键工具链路。
4. 灰度发布并监控版本维度错误率，异常可快速回退。

协议变更要像 API 变更一样管理，而不是只改一处 schema。

---

### Q19: MCP 生产中，如何落地 Tool/Resource 的协同设计？

**答案**：
把 Q11 的“概念区别”落到生产，建议用一条决策链：
1. 先判断是否有副作用。无副作用优先 Resource，有副作用用 Tool。
2. 再判断数据是否可 URI 化与缓存。可 URI 化则沉淀为 Resource，降低重复调用成本。
3. 业务动作拆成“先读后写”：Resource 提供上下文快照，Tool 执行变更并返回结构化结果。
4. 对 Tool 统一做幂等键、超时、重试和审计；对 Resource 统一做版本、缓存失效和权限隔离。

一句话：Q11 解决“怎么区分”，Q19 解决“线上怎么组合与治理”。

---

### Q20: MCP 生产排障时，最小闭环诊断路径是什么？

**答案**：
可按四步快速闭环：
1. **请求侧**：确认 client 发出的 JSON-RPC 请求结构和参数。
2. **路由侧**：确认工具发现与路由是否命中目标 Server。
3. **执行侧**：看工具耗时、错误类型、下游依赖状态。
4. **返回侧**：确认响应 schema、错误码和重试行为是否符合预期。

每一步都要有结构化日志和 trace_id，否则排障只能靠猜。

---

### 开放式设计题

**D1：设计一个企业内部的MCP Server Hub（统一管理50+内部工具），架构怎么做？**

**参考思路**：
- 注册中心：工具元数据管理（名称/描述/参数Schema/权限级别）、版本管理、健康检查
- 权限控制：按角色限制可调用的工具集、敏感操作审批流、调用审计日志
- 路由层：请求路由到对应MCP Server、负载均衡、熔断降级（工具不可用时返回友好提示）
- 可观测性：每次工具调用的延迟/成功率/参数/结果记录、异常调用告警
- 开发者体验：统一SDK、Playground测试、自动生成文档

**D2：MCP工具调用出现幻觉（LLM调用了不存在的工具或传了错误参数），如何防范？**

**参考思路**：
- 参数校验：严格的JSON Schema校验、枚举值限定、必填字段检查
- 工具描述优化：清晰的工具名称和描述、提供示例调用、明确参数约束
- 重试策略：参数校验失败后返回错误信息给LLM重新生成（max retry=2）
- 监控告警：schema validation failure率监控、异常参数Pattern统计
