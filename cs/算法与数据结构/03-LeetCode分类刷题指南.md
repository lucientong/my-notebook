...
**D1：面试前只有一周时间准备算法，如何制定最高效的刷题策略？**

**参考思路**：
- Day 1-2：高频Top 20必刷题（两数之和/反转链表/最大子数组/二叉树遍历/LRU缓存/合并区间等）
- Day 3-4：按分类刷模板题（每类1-2道：双指针/滑窗/BFS/DFS/DP基础/二分）
- Day 5-6：中等难度综合题（每天4-5道，限时30min/题，超时看题解理解思路）
- Day 7：回顾错题+高频题二刷+面试模拟（限时+口述思路）
- 关键原则：做题质量>数量、理解一种模式比做10道同类题更有效、一定要限时

**D2：面试中写完代码后面试官说"有没有更优的解法"，如何应对？**

**参考思路**：
- 先确认当前解法的复杂度，明确"更优"的方向（时间/空间/两者都要？）
- 常见优化方向：暴力O(n²)→排序+双指针O(nlogn)→哈希表O(n)、递归→迭代减少栈空间
- 思考技巧：能否用空间换时间（哈希表/前缀和）、能否减少重复计算（记忆化/DP）、能否利用数据特性（有序→二分）
- 如果想不到：诚实说"目前想到的最优是O(X)，我分析一下瓶颈在..."，展示思考过程
- 面试官可能的暗示：提到"如果数据有序呢"（二分）、"只需要Top K"（堆）、"字符集有限"（位运算/计数）
