...
### Q17: Node.js 如何避免事件循环被阻塞？

**答案**：
1. 把 CPU 密集任务移到 Worker Threads 或独立服务。
2. 避免同步阻塞 API（如 `fs.readFileSync`）出现在请求路径。
3. 大计算拆分为小任务批次（让出事件循环）。
4. 监控事件循环延迟（event loop lag）并设阈值告警。

判断标准不是 CPU 是否高，而是请求高峰时延迟是否与事件循环阻塞同步上升。

### Q18: Node.js 线上内存持续上涨时，排查顺序是什么？

**答案**：
建议固定顺序：
1. 先确认趋势：`heapUsed`、`rss`、GC 次数与停顿时间。
2. 对比多次堆快照，定位持续增长对象类型。
3. 回看引用链，确认是缓存、监听器、闭包还是全局容器未释放。
4. 用压测复现并验证修复前后曲线。

关键是“找增长对象 + 找持有关系”，不是只看某一时刻内存值。

### Q19: `process.nextTick`、Promise 微任务、`setImmediate` 应该怎么选？

**答案**：
`process.nextTick` 优先级最高，适合很短的补偿逻辑，但滥用会饿死 I/O；Promise 微任务适合标准异步链路；`setImmediate` 更适合把任务推迟到 check 阶段，让 I/O 先推进。工程上优先 Promise/`setImmediate`，仅在确有必要时使用 `nextTick`。

### Q20: Node.js 服务如何做“可回滚”的配置变更治理？

**答案**：
1. 配置版本化：每次变更有版本号和审计记录。
2. 灰度生效：按实例/流量分批放量，避免一次全量。
3. 失败自动回滚：关键指标恶化触发回退到上一个稳定版本。
4. 配置与代码兼容校验：启动时校验 schema，防止错误配置直接打挂服务。
