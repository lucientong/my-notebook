...
### Q17：React 中为什么会出现“闭包陷阱”（stale closure）？如何修复？

**答案**：
闭包陷阱本质是回调捕获了旧渲染周期的状态。常见于 `setInterval`、异步回调、事件监听。修复方式：
1. 用函数式更新：`setCount(c => c + 1)`。
2. 把依赖写全，避免 effect 使用过期变量。
3. 对长期订阅回调使用 `useRef` 或 `useEffectEvent`（团队已采用时）。

核心思想：让读取到的状态与当前渲染周期保持一致。

### Q18：什么情况下不应该使用 `useMemo`/`useCallback`？

**答案**：
这两个 Hook 不是“默认性能优化开关”。当计算本身很轻、子组件没有 `memo`、依赖频繁变化时，额外缓存和依赖比较反而增加开销。优先在有明确瓶颈的路径使用，并结合 Profiler 验证收益，而不是全量机械添加。

### Q19：React 并发渲染下，为什么副作用必须放在 Effect 而不是 render 阶段？

**答案**：
并发渲染允许 render 阶段被中断、重试、丢弃。如果在 render 里执行副作用（请求、订阅、DOM 操作），会出现重复执行和状态错乱。Effect 在 commit 后执行，才能保证对应 UI 已经落地。面试里这是区分“会写组件”和“理解渲染模型”的关键题。

### Q20：前端应用如何设计错误边界与降级策略，避免白屏扩散？

**答案**：
建议分层处理：
1. 路由级错误边界：隔离页面级崩溃。
2. 组件级错误边界：隔离高风险模块（图表、富文本、第三方组件）。
3. 全局兜底：监听未捕获错误与 Promise 拒绝并上报。
4. 降级策略：展示可操作 fallback、保留核心路径可用，而非直接白屏。
