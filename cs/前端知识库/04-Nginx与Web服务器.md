...
### Q17: `return` 和 `rewrite` 有什么区别？什么时候优先用 `return`？

**答案**：
`return` 直接返回状态码或重定向，逻辑清晰、性能开销更低；`rewrite` 用正则改写 URI，适合复杂路径重写。能用 `return 301/302` 的场景优先用 `return`，避免不必要的正则匹配和配置复杂度。

### Q18: 如何定位 Nginx 的 `499`、`502`、`504` 问题？

**答案**：
1. `499`：客户端提前断开，优先看前端超时策略、网络质量、上游响应时间。
2. `502`：上游返回异常或连接失败，检查 upstream 健康、协议匹配、网关到上游连通性。
3. `504`：网关等上游超时，重点看上游慢请求、线程池/连接池饱和、超时配置是否合理。

排查要结合 access/error log 与上游服务指标，不要只看 Nginx 单点日志。

### Q19: 为什么线上不建议滥用 `if` 指令？

**答案**：
Nginx 的 `if` 在 `location` 中语义并非通用流程控制，和很多指令组合可能出现意外行为。复杂条件建议改用 `map`、`try_files`、拆分 `location`，可读性和稳定性都更好。面试里能指出“if is evil”的边界，通常说明有实战经验。

### Q20: 反向代理链路中，如何确保真实客户端 IP 不丢失？

**答案**：
关键在于统一约定并可信传递：
1. 入口层写入 `X-Forwarded-For` / `X-Real-IP`。
2. 中间层只信任上游代理网段，避免被伪造头污染。
3. 应用层按固定规则解析首个可信 IP。
4. 日志统一记录真实 IP 字段，便于风控和排障。

如果链路中每层都“随意覆盖”头部，最终风控与审计都会失真。

---

### 开放式设计题

**D1：设计一个支持灰度发布+AB测试的Nginx流量分发方案，怎么实现？**

**参考思路**：
- 灰度维度：按Cookie（用户ID哈希）、按Header（X-Gray-Tag）、按IP段、按比例随机
- Nginx实现：split_clients按权重分流、map变量路由不同upstream、lua-nginx-module做复杂逻辑
- AB测试：请求打标（添加X-AB-Group Header）→ 透传到后端 → 后端返回不同版本 → 数据埋点对比
- 配置热更新：通过Consul/etcd动态更新upstream列表，nginx reload零停机
- 关键注意：灰度用户的Session/缓存隔离、灰度比例可观测和快速回滚

**D2：网站HTTPS证书即将过期且自动续期失败，紧急处理SOP是什么？**

**参考思路**：
- 紧急：手动certbot renew --force-renewal → nginx -t → nginx -s reload
- 排查失败原因：DNS验证失败（域名解析变更）、HTTP验证80端口不通、证书速率限制
- 长期治理：证书过期监控（30天/7天/1天三级告警）、多域名统一管理、备用证书方案
- 高可用：证书存储集中化（Vault/K8s Secret）、多节点同步、OCSP Stapling减少客户端验证延迟
